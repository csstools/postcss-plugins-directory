...
export function renderPlugin(pluginData) {
	const firstKeyword = Object.keys(interestingKeywords).find((x) => pluginData.keywords.includes(x));
	const randomHue = firstKeyword ? sfc32(
		crc32(interestingKeywords[firstKeyword]),
	)() : 13;

	return html`
<article class="plugin" id="${he.encode(encodeURIComponent(pluginData.name))}" style="--random-hue: ${(Math.round(randomHue * 36) * 10).toFixed()}">
	<details>
		<summary>
...
function sfc32(a = 0.7680775753681992, b = 0.9266817401904843, c = 0.5082854831222465, d = 0.41709149157993397) {
	return () => {
		a |= 0; b |= 0; c |= 0; d |= 0;
		const t = (a + b | 0) + d | 0;
		d = d + 1 | 0;
